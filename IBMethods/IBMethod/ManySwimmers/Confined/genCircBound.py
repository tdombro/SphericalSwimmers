...
def genBoundary(dX,definedRadius,structureName,dirName):
    #FILES
    file = open(dirName+structureName+'.vertex','w')
    
    #Plot parameters
    fig1 = plt.figure(num=1,figsize=(4,4),dpi=120)
    ax1 = fig1.add_subplot(111)
    
    #Mesh Parameters
    vertexSpace = 1.0*dX
    minRadius = definedRadius                                  #Minimum radius of disc (m)
    maxRadius = definedRadius + 3.0*dX                 #Maximum radius of disc (m)
    print((maxRadius - minRadius)/vertexSpace+1)
    #Make new dX value for Ring separation
    nRings = (maxRadius - minRadius)/vertexSpace + 1       #Number of rings in disc
    print('b4: dX = ',dX)
    ...
    for i in range(nRings):
        ...
        for j in range(npts):
            ...
            arrVertex[j,0] = radius*np.cos(j*dtheta)
            arrVertex[j,1] = radius*np.sin(j*dtheta)
    ...
    #ax1.axis('equal')
    ax1.axis([-0.046,-0.044,-0.001,0.001])
    fig1.savefig(structureName+'.png',bbox_inches='tight',pad_inches=0.5)
    fig1.clf()
    #plt.show()
    plt.close()
...
def genDiscMesh(dX,definedRadius,origin,structureName,dirName):
    ...
    print((maxRadius - minRadius)/minRadius+1)
    #Make new dX value for Ring separation
    nRings = (maxRadius - minRadius)/minRadius + 1       #Number of rings in disc
    print('b4: dX = ',dX)
    #dX = 2.0*(maxRadius-minRadius)/nRings
    dX *= nRings/int(np.ceil(nRings))
    print('a4: dX = ',dX)
...
def genDiscSprings(pointArr,structureName,dirName):    
    ...
    ax2.axis('equal')   
    fig2.savefig(structureName+'Springs.png')
    fig2.clf()
    plt.close()
    return
    
if __name__ == '__main__':
    #Goal: Generate a uniform 2D Ring mesh from a loop of rings
    ...
    #Disc Parameters
    boundRadii = 8.0*0.005625
    structureName = 'boundary'
    dirName = 'RotatedDisc/44/27510/'
                
    genBoundary(dX,boundRadii,structureName,dirName)
