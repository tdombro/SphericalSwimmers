...
#include <ibtk/LDataManager.h> //Added for MODULE 2016-11
...
void
output_cm_data(Pointer<PatchHierarchy<NDIM> > patch_hierarchy,
               LDataManager* l_data_manager,
               const int iteration_num,
               const double loop_time,
               std::ostream& dumFile);
void
postprocess_force(Pointer<PatchHierarchy<NDIM> > patch_hierarchy,
                  LDataManager* l_data_manager,
                  const double loop_time,
                  ostream& F_stream,
                  const int nodes_cyl);
    ...
    char* argv[])
{
    ...
    {// cleanup dynamically allocated objects prior to shutdown
        ...
        const int viz_dump_interval_SS = int(0.005/dt)+1;
        const int viz_dump_interval_NoSS = int(0.1/dt);
        double SSTIME_START = 0.0;
        double SSTIME_END = SSTIME_START + 0.2;
        //Swimmer Parameters
        const int nodes_cyl = 1746;
        ...
        {
            F_stream.open("fd.txt", ios_base::out | ios_base::trunc);
            F_stream << "F_x F_y time" << "\n" << std::flush;
            position_data.open("pd.txt", ios::out | ios::trunc);
            position_data << "idx x y time\n" << std::flush;
        }
        ...
        while (!MathUtilities<double>::equalEps(loop_time,loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            //VisIt DUMPS
            //Begin Changes
            if(loop_time <= SSTIME_START || loop_time > SSTIME_END)
            {
                if (dump_viz_data && uses_visit && (iteration_num % viz_dump_interval_NoSS == 0 || last_step))
                {
                    pout << "\nWriting visualization files...\n\n";
                    time_integrator->setupPlotData();
                    visit_data_writer->writePlotData(patch_hierarchy, iteration_num, loop_time);
            ...
            //Second Part of Data Output (SSTIME_START -> SSTIME_END)
            if(loop_time > SSTIME_START && loop_time <= SSTIME_END)
            {
                if (dump_viz_data && uses_visit && (iteration_num % viz_dump_interval_SS == 0 || last_step))
                {
                    pout << "\nWriting visualization files...\n\n";
                    time_integrator->setupPlotData();
                    visit_data_writer->writePlotData(patch_hierarchy, iteration_num, loop_time);
            ...
            if(loop_time >= SSTIME_END)
            {
                SSTIME_START = SSTIME_START + 1.0;
                SSTIME_END = SSTIME_START + 0.2;
            }
            //End VisIt dump changes
            ...
            if (dump_postproc_data && (iteration_num%postproc_data_dump_interval == 0 || last_step))
            {
                ...
                output_cm_data(patch_hierarchy, l_data_manager, iteration_num, loop_time, position_data);
                postprocess_force(patch_hierarchy, l_data_manager, loop_time, F_stream, nodes_cyl);
            }
               ...
               std::ostream& dumFile)
{
    ...
    std::vector<LNode*> nodes = l_mesh->getLocalNodes();
    for (const auto& node: nodes)
    {
        ...
        if(lag_idx == 0)
        {
            ...
            dumFile << lag_idx << " " << X(0) << " " << X(1) << " " << loop_time << "\n" << std::flush;
        }
    }
    return;
}
                  ...
                  const int nodes_cyl)
{
    ...
    const boost::multi_array_ref<double, NDIM>& F_arr = *F_data->getLocalFormVecArray();
    double F[NDIM];
    ...
    if (SAMRAI_MPI::getRank() == 0)
    {
        F_stream.precision(12);
        F_stream.setf(ios::fixed, ios::floatfield);
        F_stream << F[0] << " " << F[1] << " " << std::flush;
    }
