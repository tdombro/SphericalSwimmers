...
def GetAvgFieldData(cwd,idx):
    ...
    ayList = fieldData['ayavg'].values.tolist()
...
if __name__ == '__main__':
    ...
    for countPer in range(nPer):
        #strDir = cwd_PYTHON+"../Figures/AVG/"+Re+"/avgU/"
        #AVGPlot = pathlib.Path(strDir+'avgU_%i.png'%countPer)
        AVGFile = pathlib.Path(cwd_DATA+'AVG_%04d.csv'%countPer)
        ...
        if AVGFile.exists ():
            ...
            #Combination of Vorticity and Streamlines
            pars[1] = 'stream'
            ...
            pars[1] = 'force'
            PlotForceField(cwd_PYTHON,time,mx,my,avgFx,avgFy,posData,pars)
            pars[1] = 'forcestream'
