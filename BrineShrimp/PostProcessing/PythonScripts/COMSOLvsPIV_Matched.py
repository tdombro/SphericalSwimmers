...
def getCOMSOLInputData(meshName):
    ...
    cwd_DATA = cwd_PYTHON + '/../COMSOL/MatchedResults/'+meshName+'/'
...
def PlotSliceVelData(meshName,sliceLoc,velString,PIVdata,COMSOLdata,Vmin,Vmax):
    ...
    PLOTNUM = 0
    #Not Normalized
    pathlib.Path('../Images/Matched/Slice/Raw/'+velString).mkdir(parents=True, exist_ok=True)
    cwd_SLICE = cwd_PYTHON + '/../Images/Matched/Slice/Raw/'+velString+'/'
    #Normalized
    #pathlib.Path('../Images/Matched/Slice/Normalized/'+velString).mkdir(parents=True, exist_ok=True)
    #cwd_SLICE = cwd_PYTHON + '/../Images/Matched/Slice/Normalized/'+velString+'/'
    #Create figure and axis
...
def PlotLineoutData(meshName,sliceLoc,PIVdata,COMSOLdata):
    ...
    #of the mesh
    #Not Normalized
    pathlib.Path('../Images/Matched/Lineout/Raw').mkdir(parents=True, exist_ok=True)
    cwd_LINE = cwd_PYTHON + '/../Images/Matched/Lineout/Raw/'
    #Normalized
    #pathlib.Path('../Images/Matched/Lineout/Normalized').mkdir(parents=True, exist_ok=True)
    #cwd_LINE = cwd_PYTHON + '/../Images/Matched/Lineout/Normalized/'
    #Create figure and axis
    fig = plt.figure(num=FIGNUM,figsize=(4,3),dpi=500)
    ax = fig.add_subplot(111)
    #csfont = {'fontname':'Times New Roman'}
    csfont = {'fontname':'Arial'}
    #ax.set_title(meshName+': '+sliceLoc+' pegs',**csfont,fontsize=16)
    ax.set_xlabel('height (mm)',fontsize=12,**csfont)
    ax.set_ylabel(r'$U_x$ ($\frac{mm}{s}$)',fontsize=12,**csfont)
    ...
    if(meshName == '10x20x2'):
        #Red Lines
        ax.plot(COMSOLdb.z,COMSOLdb.V*1.0e3,color='r',linewidth=1.5,label='COMSOL')
        #ax.scatter(COMSOLdb.z,COMSOLdb.V,color='r',s=9,label='')
        #Dashed for PIV
        ax.plot(PIVdb.z,PIVdb.V*1.0e3,color='r',ls='--',linewidth=1.5,label='PIV')
        #ax.scatter(PIVdb.z,PIVdb.V,color='b',s=9,label='')
    else:
        ax.plot(COMSOLdb.z,COMSOLdb.V*1.0e3,color='k',linewidth=1.5,label='COMSOL')
        ax.plot(PIVdb.z,PIVdb.V*1.0e3,color='k',ls='--',linewidth=1.5,label='PIV')
    #Vmin = np.amin(COMSOLdata)
    Vmax = np.nanmax(COMSOLdata) + 0.01
    #Not Normalized
    ax.axis([0,80,0.0,75.0])
    #Normalized
    #ax.axis([0,80,0.0,1.0])
    SetAxesParameters(ax,0.0,80.0,10.0,5.0,5.0)
    ax.legend(loc='best',fontsize='small')
    fig.tight_layout()
    fig.savefig(cwd_LINE + meshName+'_'+sliceLoc+'.png')
    fig.savefig(cwd_LINE + meshName+'_'+sliceLoc+'.svg')
...
def PlotVelSpectrum(meshName,sliceLoc,PIVdata,COMSOLdata):
    ...
    #Not Normnalized
    pathlib.Path('../Images/Matched/VelDist/Raw').mkdir(parents=True, exist_ok=True)
    cwd_HIST = cwd_PYTHON + '/../Images/Matched/VelDist/Raw/'
    #Normalized
    #pathlib.Path('../Images/Matched/VelDist/Normalized').mkdir(parents=True, exist_ok=True)
    #cwd_HIST = cwd_PYTHON + '/../Images/Matched/VelDist/Normalized/'
    #Create figure and axis
    ...
    axP.set_xlabel(r'V ($\frac{m}{s}$)',fontsize=14,**csfont)
    ...
    #Not Normalized
    axP.axis([0.0,0.075,0.0,0.35])
    axC.axis([0.0,0.075,0.0,0.35])
    #Normalized
