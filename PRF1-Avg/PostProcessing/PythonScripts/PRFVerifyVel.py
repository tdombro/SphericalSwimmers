...
def main():
    ...
    file.write('visc ampFrac rslFrac vel t_ss\n')
        ...
        for dirVisc in dirsVisc:
            ...
            for dirAmp in dirsAmp:
                ...
                if(np.amax(data.time) > 1.0):
                    ...
                    fig = plt.figure(num=fignum,figsize=(4,4),dpi=200)
                    ...
                    #Plot MaxVel Slope during interval
                    xval = np.linspace(t_ss,t_ss+10.0*PERIOD,100)
                    yval = maxSlope*xval + maxIntercept
                    ax.plot(xval,yval,'r')
                    ax.plot(xval,yval - RSL_DEFAULT*float(rslValue),'r')
                    fig.savefig('SteadyState'+rsl+'.'+visc+'.'+amp+'.png')
                    fig.clf()
                    fignum+=1
                    ...
                    file.write('%.5e %.5e %.5e %.5e %.5e\n' %(float(viscValue),float(ampValue),float(rslValue),maxSlope, t_ss))
                    ...
                    MakeDirectory(cwd_RSL,'PRFVerify/'+rsl+'/'+visc)
                    cwd_IMAGE = cwd_RSL+'/PRFVerify/'+rsl+'/'+visc+'/'
                    copyfile('SteadyState'+rsl+'.'+visc+'.'+amp+'.png', cwd_IMAGE+'SteadyState'+rsl+'.'+visc+'.'+amp+'.png')
                    #return
                else:
                    maxSlope = 'NA'
                    t_ss = 'NA'
                    file.write('%.5e %.5e %.5e %s %s\n' %(float(ReValue),float(ampValue),float(rslValue),maxSlope,t_ss))
                    dir_end = perf_counter()
                    print('Processed {} in {:.3f} seconds\n'.format(dirRSL+'/Structures/'+dirVisc+'/'+dirAmp, dir_end - dir_start))
