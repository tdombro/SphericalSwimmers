...
def GetAxisBounds(pos,xCM0,yCM0):
    ...
    if(maxSwim + 0.01 > maxWin): #Swimmer is leaving window area. Make it larger
        ...
        minWin = -1.0*maxWin
        print("\n\nmaxWin = %.3f\n\n"%maxWin)
    return [minWin+xCM0,maxWin+xCM0,minWin+yCM0,maxWin+yCM0]
...
def PlotCombinedStream(cwd,time,mx,my,w,Ux,Uy,pos,pars,xCM0,yCM0):
    Re = pars[0]
    config = pars[1]
    field = pars[2]
    idx = pars[3]
    ...
    xCM = 0.25*(pos.loc[0,'aXU'] + pos.loc[0,'bXU'] + pos.loc[0,'aXL'] + pos.loc[0,'bXL'])
    yCM = 0.25*(pos.loc[0,'aYU'] + pos.loc[0,'bYU'] + pos.loc[0,'aYL'] + pos.loc[0,'bYL'])
    #ax.scatter([xCM],[yCM],color='purple',s=20)
    ...
    axis = GetAxisBounds(pos,xCM0,yCM0)
    ax.axis(axis)
    ax.set_aspect('equal')
    ...
    for countPer in range(nPer):
        if(countPer <= 200):
            ...
            if AVGPlot.exists ():
                ...
                if(countPer == 0):
                    xCM0 = 0.25*(posData.loc[0,'aXU'] + posData.loc[0,'bXU'] + posData.loc[0,'aXL'] + posData.loc[0,'bXL'])
                    yCM0 = 0.25*(posData.loc[0,'aYU'] + posData.loc[0,'bYU'] + posData.loc[0,'aYL'] + posData.loc[0,'bYL'])
                #print(posData)
                #Plot Averaged Field Data
                #Vorticity And Streamlines
                pars = [Re,config,'Moving',countPer]
                PlotCombinedStream(cwd_PYTHON,time,mx,my,avgW,avgUx,avgUy,posData,pars,xCM0,yCM0)
