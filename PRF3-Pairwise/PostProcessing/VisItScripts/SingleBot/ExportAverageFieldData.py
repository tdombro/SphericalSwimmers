...
if __name__ == '__main__':
    ...
    for countPer in range(nPer):
        ...
        if not AVGfile.exists ():
            ...
            uxPrev = np.zeros((Nx,Ny))
            uyPrev = np.zeros((Nx,Ny))
            axavg = np.zeros((Nx,Ny))
            ayavg = np.zeros((Nx,Ny))
            ...
            for idx in range(0,int(DUMP_INT)):
                ...
                if(idx == 0 and countPer > 0):
                    axavg = (uxArr - uxPrev)/PERIOD
                    ayavg = (uyArr - uyPrev)/PERIOD
                    uxPrev = copy.deepcopy(uxArr)
                    uyPrev = copy.deepcopy(uyArr)
            ...
            avgDict = {'mx':mxavg.flatten(),'my':myavg.flatten(),'avgW':wavg.flatten(),
                       ...
                       'avgax':axavg.flatten(),'ayavg':ayavg.flatten()
                      }
