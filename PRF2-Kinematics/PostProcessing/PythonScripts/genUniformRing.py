...
def genDiscMesh(dX,definedRadius,origin,structureName,dirName):
    ...
    #Mesh Parameters
    minRadius = 2.0*dX                                  #Minimum radius of disc (m)
    maxRadius = definedRadius # - 1.25*dX                 #Maximum radius of disc (m)
    print((maxRadius - minRadius)/(2.0*dX)+1)
    #Make new dX value for Ring separation
    nRings = (maxRadius - minRadius)/(2.0*dX) + 1       #Number of rings in disc
    print('b4: dX = ',dX)
    ...
    for i in range(nRings-1,nRings):
        #Ring Parameters
        radius = (i+1)*2.0*dX
        #radius = minRadius + i*0.5*dX                   #Radius of band (m)
        bandLength = 2.0*np.pi*radius                   #Circumference of band (m)
        npts = m.ceil((bandLength/(2.0*dX)))  #Number of vertex points along band
...
def main():
    ...
    structureNames = ['ringup1','ringlow1']
    restSpringLength = 1.3*(np.sum(listRadii) + 0.30)
