...
if __name__ == '__main__':
    ...
    allData = pd.read_csv(cwd_PYTHON+'/LabTrajData_Re10.csv',delimiter=' ')

    allData['xCM'] = 0.5*(allData['aXCM'] + allData['bXCM'])
    allData['yCM'] = 0.5*(allData['aYCM'] + allData['bYCM'])
    allData['aXChange'], allData['aYChange'] = 0.0, 0.0
    allData['bXChange'], allData['bYChange'] = 0.0, 0.0
    
    #CALCULATE LAB FRAME Angles
    allData['aLabX'] = allData['aXU'] - allData['aXL']
    allData['aLabY'] = allData['aYU'] - allData['aYL']
    allData['bLabX'] = allData['bXU'] - allData['bXL']
    allData['bLabY'] = allData['bYU'] - allData['bYL']
    allData['aLength'] = np.hypot(allData['aLabX'],allData['aLabY'])
    allData['bLength'] = np.hypot(allData['bLabX'],allData['bLabY'])
    allData['aNormX'] = allData['aLabX']/allData['aLength']
    allData['aNormY'] = allData['aLabY']/allData['aLength']
    allData['bNormX'] = allData['bLabX']/allData['bLength']
    allData['bNormY'] = allData['bLabY']/allData['bLength']
    #Swimmer A Angle
    allData['aTheta'] = np.arccos(allData['aNormX'])
    thetaValues = allData['aTheta'].values
    is_cc = allData['aNormY'] < 0
    thetaValues[is_cc] = 2.0*np.pi - thetaValues[is_cc]
    allData['aTheta'] = thetaValues
    #Swimmer B Angle
    allData['bTheta'] = np.arccos(allData['bNormX'])
    thetaValues2 = allData['bTheta'].values
    is_cc2 = allData['bNormY'] < 0
    thetaValues2[is_cc2] = 2.0*np.pi - thetaValues2[is_cc2]
    allData['bTheta'] = thetaValues2
    ...
    for idx in range(len(parHxList)):
        ...
        if Theta == ThetaValue:
            ...
            for idx in range(len(HyData['time'])):
                ...
                if idx >= 1:
                    #print(HyData['time'])
                    #Shift center of mass values
                    HyData.loc[idx,'aXChange'] = HyData.loc[idx,'aXCM'] - HyData.loc[idx-1,'aXCM']
                    HyData.loc[idx,'aYChange'] = HyData.loc[idx,'aYCM'] - HyData.loc[idx-1,'aYCM']
                    HyData.loc[idx,'bXChange'] = HyData.loc[idx,'bXCM'] - HyData.loc[idx-1,'bXCM']
                    HyData.loc[idx,'bYChange'] = HyData.loc[idx,'bYCM'] - HyData.loc[idx-1,'bYCM']
            HyData['aXShift'], HyData['aYShift'] = 0.0, 0.0
            HyData['bXShift'], HyData['bYShift'] = 0.0, 0.0
            #print(HyData.head(10))
            #print(HyData['aNormX'].head(10))
            #print(HyData['aNormY'].head(10))
            #print(180.0*HyData['aTheta'].head(10)/np.pi)
            #print(HyData['bNormX'].head(10))
            #print(HyData['bNormY'].head(10))
            #print(180.0*HyData['bTheta'].head(10)/np.pi)
            #sys.exit(0)
            
            strMovie = cwd_PYTHON+"/../Figures/LabTrajectories_Shifted/Re10/T{0}/TrajMov_T{1}_Hx{2}_Hy{3}_.mp4".format(Theta,Theta,Hx,Hy)
            boolMovie = os.path.isfile(strMovie)
            print('boolMovie = ',boolMovie)
            if boolMovie == False:
                for idxTime in range(len(HyData['time'])):
                    if HyData.loc[idxTime,'aXChange']*HyData.loc[idxTime,'bXChange'] >= 0.0:
                        #Shift is in same direction
                        if abs(HyData.loc[idxTime,'aXChange']) >= abs(HyData.loc[idxTime,'bXChange']):
                            HyData.loc[idxTime,'aXShift'] = HyData.loc[idxTime,'aXChange'] - HyData.loc[idxTime,'bXChange']
                        else:
                            HyData.loc[idxTime,'bXShift'] = HyData.loc[idxTime,'bXChange'] - HyData.loc[idxTime,'aXChange']
                    else:
                        #Shift is in different direction
                        HyData.loc[idxTime,'aXShift'] = HyData.loc[idxTime,'aXChange']
                        HyData.loc[idxTime,'bXShift'] = HyData.loc[idxTime,'bXChange']
                    
                    if HyData.loc[idxTime,'aYChange']*HyData.loc[idxTime,'bYChange'] >= 0.0:
                        #Shift is in same direction
                        if abs(HyData.loc[idxTime,'aYChange']) >= abs(HyData.loc[idxTime,'bYChange']):
                            HyData.loc[idxTime,'aYShift'] = HyData.loc[idxTime,'aYChange'] - HyData.loc[idxTime,'bYChange']
                        else:
                            HyData.loc[idxTime,'bYShift'] = HyData.loc[idxTime,'bYChange'] - HyData.loc[idxTime,'aYChange']
                    else:
                        #Shift is in different direction
                        HyData.loc[idxTime,'aYShift'] = HyData.loc[idxTime,'aYChange']
                        HyData.loc[idxTime,'bYShift'] = HyData.loc[idxTime,'bYChange']

                    #Shift CM and sphere center locations
                    #CM
                    if idxTime >= 1:
                        HyData.loc[idxTime,'aXCM'] = HyData.loc[idxTime-1,'aXCM'] + HyData.loc[idxTime,'aXShift']
                        HyData.loc[idxTime,'aYCM'] = HyData.loc[idxTime-1,'aYCM'] + HyData.loc[idxTime,'aYShift']
                        HyData.loc[idxTime,'bXCM'] = HyData.loc[idxTime-1,'bXCM'] + HyData.loc[idxTime,'bXShift']
                        HyData.loc[idxTime,'bYCM'] = HyData.loc[idxTime-1,'bYCM'] + HyData.loc[idxTime,'bYShift']
                    #Upper Sphere
                    HyData.loc[idxTime,'aXU'] = HyData.loc[idxTime,'aXCM'] + 0.5*np.cos(HyData.loc[idxTime,'aTheta'])
                    HyData.loc[idxTime,'aYU'] = HyData.loc[idxTime,'aYCM'] + 0.5*np.sin(HyData.loc[idxTime,'aTheta'])
                    HyData.loc[idxTime,'bXU'] = HyData.loc[idxTime,'bXCM'] + 0.5*np.cos(HyData.loc[idxTime,'bTheta'])
                    HyData.loc[idxTime,'bYU'] = HyData.loc[idxTime,'bYCM'] + 0.5*np.sin(HyData.loc[idxTime,'bTheta'])
                    #Lower Sphere
                    HyData.loc[idxTime,'aXL'] = HyData.loc[idxTime,'aXCM'] - 2.0*np.cos(HyData.loc[idxTime,'aTheta'])
                    HyData.loc[idxTime,'aYL'] = HyData.loc[idxTime,'aYCM'] - 2.0*np.sin(HyData.loc[idxTime,'aTheta'])
                    HyData.loc[idxTime,'bXL'] = HyData.loc[idxTime,'bXCM'] - 2.0*np.cos(HyData.loc[idxTime,'bTheta'])
                    HyData.loc[idxTime,'bYL'] = HyData.loc[idxTime,'bYCM'] - 2.0*np.sin(HyData.loc[idxTime,'bTheta'])
                    '''
                    if idxTime == 200:
                        print(HyData.tail(10))
                        print(HyData['aYShift'].tail(20))
                        print(HyData['bYShift'].tail(20))
                        #sys.exit(0)
                    '''
                    ...
                    #plt.show()
                    #sys.exit(0)
                    strDir = cwd_PYTHON+"/../Figures/LabTrajectories_Shifted/Re10/T{0}/".format(Theta)
                    ...
                    if idxTime % 20 == 0:
                        print('time = ',time)
                os.chdir(cwd_PYTHON+"/../Figures/LabTrajectories_Shifted/Re10/T{0}/".format(Theta))
