    ...
    const double dt)
{
    ...
    static const double L1 = 0.05; // length of computational domain (meters)
    static const int N1 = 1024; // number of cartesian grid meshwidths at the finest level of the AMR grid
    static const double freq = 10.0; //Frequency of oscillation
    static const double omega = 2.0*freq*M_PI; //Angular frequency
    ...
    static const double upper_radius = 0.001*2.0 - 1.25*dX;
	...
	/*for(int i=0;i<9;i++){
	  if (lag_idx == 2 + nvert*i){
	    ...
	    spring0[1] = 0.375 + 0.15*0.8*sin(2.0*M_PI*10.0*(current_time+dt));
	    pout << "a4: resting_length = " << spring0[1] << "\n";
	  }
	  
	  if(lag_idx == 13 + nvert*i){
	    pout << "b4: lag_idx = " << lag_idx << "\tresting_length = " << spring0[1] << "\n";
	    pout << "resting_length contains " << spring0.size() << " elements \n";
	    xLength = upper_half_radius - lower_radius;
	    yLength = 0.375 + 0.15*0.8*sin(2.0*M_PI*10.0*(current_time+dt));
	...
	if(lag_idx == 6){
	  pout << "b4: lag_idx = " << lag_idx << "\tresting_length = " << spring0[1] << "\n";
	  pout << "resting_length contains " << spring0.size() << " elements \n";
	  spring0[1] = 0.005 + amp*sin(omega*(current_time+dt));
	  pout << "a4: resting_length = " << spring0[1] << "\n";
	}
	...
	if(lag_idx == 2){
	  ...
	  pout << "resting_length contains " << spring0.size() << " elements \n";
	  xLength = upper_half_radius - lower_radius;
	  yLength = 0.005 + amp*sin(omega*(current_time+dt));
	  spring0[1] = sqrt(xLength*xLength + yLength*yLength);
	  pout << "a4: resting_length = " << spring0[1] << "\n";
	  std::vector<double>& spring1 = spring_spec->getParameters()[1];
	  pout << "b4: lag_idx = " << lag_idx << "\tresting_length = " << spring1[1] << "\n";
