...
def StoreData(strR,strTheta, strConfig, strRe):
    ...
    #Load position data
    #Periodic
    print(strR+'/'+strTheta+'/'+strConfig+'/'+strRe)
    if(strR == '3' and strTheta == 'PI0' and strConfig == 'Perp' and strRe == 'Stat'):
        return
    pdData = pd.read_csv(cwd_PYTHON+'/../Periodic/EqualSpheres/PhiPI/'+strR+'/'+strTheta+'/'+strConfig+'/'+strRe+'/pd.txt',delimiter = ' ')
    #Create CM variables
    pdData["aXCM"] = 0.5*(pdData.aXU + pdData.aXL)
    pdData["aYCM"] = 0.5*(pdData.aYU + pdData.aYL)
    pdData["bXCM"] = 0.5*(pdData.bXU + pdData.bXL)
    pdData["bYCM"] = 0.5*(pdData.bYU + pdData.bYL)
...
def PlotTrajectory(data,strR,strTheta,strConfig,strRe):
    ...
    #Periodic
    pathlib.Path('../TrajectoryImages/Periodic/EqualSpheres/PhiPI/'+strR+'/'+strConfig+'/').mkdir(parents=True, exist_ok=True)
    cwd_TRAJ = cwd_PYTHON + '/../TrajectoryImages/Periodic/EqualSpheres/PhiPI/'+strR+'/'+strConfig+'/'
    #GENERATE FIGURE
    ...
    ax.set_ylabel('y (R)',fontsize=14,**csfont)
    ax.set_xlabel('x (R)',fontsize=14,**csfont)
    ax.scatter(data.aXCM/RADIUSLARGE,data.aYCM/RADIUSLARGE,c=data.time,cmap='viridis',s=1)
    ax.scatter(data.bXCM/RADIUSLARGE,data.bYCM/RADIUSLARGE,c=data.time,cmap='rainbow',s=1)
    ...
    fig.tight_layout()
    fig.savefig(cwd_TRAJ+'Traj_ESPI_'+strR+'R_'+strTheta+'_'+strConfig+'_'+strRe+'.png')
    fig.clf()
    plt.close()
    return
...
def PlotDistAngle(distU,angleU,distL,angleL,distCM,angleCM,time,strR,strTheta,strConfig,strRe):
    ...
    #Periodic
    pathlib.Path('../DistAnglePlots/Periodic/EqualSpheres/PhiPI/'+strR+'/'+strConfig+'/').mkdir(parents=True, exist_ok=True)
    cwd_DIST = cwd_PYTHON + '/../DistAnglePlots/Periodic/EqualSpheres/PhiPI/'+strR+'/'+strConfig+'/'
    #GENERATE FIGURE
    csfont = {'fontname':'Times New Roman'}
    ...
    fig.tight_layout()
    fig.savefig(cwd_DIST+'Dist_ESPI_'+strR+'R_'+strTheta+'_'+strConfig+'_'+strRe+'.png')
    fig.clf()
    plt.close()
    return
...
if __name__ == '__main__':
    ...
    for idxR in range(len(R)):
        cwd_R = cwd_PYTHON + '/../Periodic/EqualSpheres/PhiPI/' + R[idxR]
