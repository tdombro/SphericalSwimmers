...
def StoreData(strR,strTheta, strConfig, strRe):
    ...
    #Load position data
    #Periodic
    pdData = pd.read_csv(cwd_PYTHON+'/../Periodic/EqualSpheres/'+strR+'/'+strTheta+'/'+strConfig+'/'+strRe+'/pd.txt',delimiter = ' ')
    #Create CM variables
...
def PlotTrajectory(data,strR,strTheta,strConfig,strRe):
    ...
    #Periodic
    pathlib.Path('../TrajectoryImages/Periodic/EqualSpheres/'+strR+'/'+strConfig+'/').mkdir(parents=True, exist_ok=True)
    cwd_TRAJ = cwd_PYTHON + '/../TrajectoryImages/Periodic/EqualSpheres/'+strR+'/'+strConfig+'/'
    #GENERATE FIGURE
    ...
    fig.tight_layout()
    fig.savefig(cwd_TRAJ+'Traj_ES_'+strR+'R_'+strTheta+'_'+strConfig+'_'+strRe+'.png')
    fig.clf()
    plt.close()
    return
...
def PlotDistAngle(distU,angleU,distL,angleL,distCM,angleCM,time,strR,strTheta,strConfig,strRe):
    ...
    #Periodic
    pathlib.Path('../DistAnglePlots/Periodic/EqualSpheres/'+strR+'/'+strConfig+'/').mkdir(parents=True, exist_ok=True)
    cwd_DIST = cwd_PYTHON + '/../DistAnglePlots/Periodic/EqualSpheres/'+strR+'/'+strConfig+'/'
    #GENERATE FIGURE
    csfont = {'fontname':'Times New Roman'}
    ...
    fig.tight_layout()
    fig.savefig(cwd_DIST+'Dist_ES_'+strR+'R_'+strTheta+'_'+strConfig+'_'+strRe+'.png')
    fig.clf()
    plt.close()
    return
...
if __name__ == '__main__':
    ...
    for idxR in range(len(R)):
        cwd_R = cwd_PYTHON + '/../Periodic/EqualSpheres/' + R[idxR]
