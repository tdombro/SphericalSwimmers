    ...
    char* argv[])
{
    ...
    {// cleanup dynamically allocated objects prior to shutdown
        ...
        double dt = 0.0;
        const int viz_dump_interval_SS = int(0.005/1.0e-5);
        const int viz_dump_interval_NoSS = int(0.1/1.0e-5);
        const int hier_dump_interval_SS = int(10.0);
        const int hier_dump_interval_NoSS = int(0.05/1.0e-5);
        const double SSTIME_START = 3.8;
        const double SSTIME_END = SSTIME_START + 0.2;
        ...
        while (!MathUtilities<double>::equalEps(loop_time,loop_time_end) && time_integrator->stepsRemaining())
        {
            ...
            //EDITED FOR SPECIFIC DUMP INTERVALS
            ...
            //VISIT DATA OUTPUT                                                   
            //First Part of Data Output (0 -> SSTIME_START or SSTIME_END -> 10s)  
            if(loop_time <= SSTIME_START or loop_time > SSTIME_END)
            {
                ...
                //HIERARCHY DATA OUTPUT
                if (dump_postproc_data && (iteration_num%hier_dump_interval_NoSS == 0 || last_step))
                {
            ...
            if(loop_time > SSTIME_START and loop_time <= SSTIME_END)
            {
                ...
                //HIERARCHY DATA OUTPUT
                if (dump_postproc_data && (iteration_num%hier_dump_interval_SS == 0 || last_step))
                {
                    output_data(patch_hierarchy, navier_stokes_integrator, l_data_manager, iteration_num, loop_time, postproc_data_dump_dirname);
                }
            }
	    ...
	    //END EDITED FOR SPECIFIC DUMP INTERVALS
