...
'''def FindDistBW(aXU,aYU,bXU,bYU,aXL,aYL,bXL,bYL):
    #Find Distance b/w the 2 swimmers (Large Spheres)
    distXU = bXU - aXU
    distYU = bYU - aYU
    distBW = np.sqrt(distXU*distXU + distYU*distYU)
    #Find relative heading Theta (angle formed by 2 swimmers)
    dist_aX = aXU - aXL
    dist_aY = aYU - aYL
    dist_a = np.sqrt(dist_aX*dist_aX + dist_aY*dist_aY)
    dist_bX = bXU - bXL
    dist_bY = bYU - bYL
    dist_b = np.sqrt(dist_bX*dist_bX + dist_bY*dist_bY)
    dotX = dist_bX*dist_aX
    dotY = dist_bY*dist_aY
    angleBW = np.arccos((dotX+dotY)/(dist_a*dist_b))
    #print(dist_aY.head())
    #print(dist_aX.head())
    #print(dist_bY.head())
    #print(dist_bX.head())
    #print(angleBW.head()*180.0/np.pi)
    #sys.exit(0)
